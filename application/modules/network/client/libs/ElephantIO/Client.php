...
class Client {
	...
	public function keepAlive($listen = false) {
		if($listen){
			...
			if($res !== true) $this->log('error', $res);
		}
	...
	public function send($type, $ns = null, $event = null, $data = null) {
		...
		$raw_message = $type;
		if($ns && $ns != '/') $raw_message .= $ns;
		if($event){
			$raw_message .= ',["'.$event.'"';
			if(!empty($data)) $raw_message .= ','.json_encode($data);
			$raw_message .= ']';
		}
	...
	private function response($raw_str){
		...
		if(!empty($service_arr[0])) $return['type'] = $service_arr[0];
		if(!empty($service_arr[1])) $return['namespace'] = $service_arr[1];
		if(!empty($event[0])) $return['event'] = $event[0];
		if(!empty($event[1])) $return['data'] = (array)$event[1];
...
class commandSocket {
	...
	public function __construct($address=false, $port=false, $type='file', $eventsDir = null){
		if($address != false) $this->address = $address;
	...
	private function parseStr($str){
		...
		if(isset($parts[1]) && !empty($parts[1])) $data['params'] = json_decode($parts[1], true);
